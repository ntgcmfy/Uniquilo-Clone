...
import { ArrowRight, Truck, Shield, RefreshCw, Camera, Sparkles } from 'lucide-react';
...
const Home: React.FC = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.error('Không thể tải sản phẩm cho trang chủ', error);
      }
    })();
  ...
  const featuredProducts = useMemo(
    () => catalog.filter((p) => p.rating && p.rating >= 4.5).slice(0, 8),
    [catalog]
  );
  const newProducts = useMemo(() => catalog.filter((p) => p.isNew), [catalog]);
  const saleProducts = useMemo(() => catalog.filter((p) => p.isSale), [catalog]);
  const menProducts = useMemo(() => catalog.filter((p) => p.category === 'men').slice(0, 4), [catalog]);
  const womenProducts = useMemo(
    () => catalog.filter((p) => p.category === 'women').slice(0, 4),
    [catalog]
  );
  const kidsProducts = useMemo(
    () => catalog.filter((p) => p.category === 'kids').slice(0, 4),
    [catalog]
  );
