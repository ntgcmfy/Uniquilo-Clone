...
const ProductDetail: React.FC = () => {
  ...
  const [relatedProducts, setRelatedProducts] = useState(
    fallbackProduct
      ? products.filter((p) => p.category === fallbackProduct.category && p.id !== fallbackProduct.id).slice(0, 4)
      : []
  );
  ...
  useEffect(() => {
    ...
    const fetchProduct = async () => {
      ...
      const fallbackRelated = fallbackProduct
        ? products
      ...
      try {
        ...
        if (remoteProduct) {
          ...
          const remoteRelated = await getRelatedProducts(remoteProduct.category, remoteProduct.id);
          if (!isMounted) return;
          setRelatedProducts(remoteRelated.length > 0 ? remoteRelated : fallbackRelated);
        } else if (fallbackProduct) {
          setProduct(fallbackProduct);
          setRelatedProducts(fallbackRelated);
          setError('Không tìm thấy sản phẩm trong Supabase, đang hiển thị dữ liệu demo.');
        } else {
          setProduct(null);
          setRelatedProducts([]);
        ...
        if (fallbackProduct) {
          setProduct(fallbackProduct);
          setRelatedProducts(fallbackRelated);
          setError('Không thể tải dữ liệu từ Supabase, đang hiển thị dữ liệu demo.');
        } else {
          setProduct(null);
          setRelatedProducts([]);
  ...
  const handleAddToCart = () => {
    if (product && selectedColor && selectedSize) {
      ...
      showToast('error', 'Vui lòng chọn màu và kích thước trước khi thêm vào giỏ hàng');
    }
  };
