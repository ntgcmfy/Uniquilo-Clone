...
import { filterProducts, SortOption } from '../utils/searchResults';
...
const Search: React.FC = () => {
    ...
    (async () => {
      ...
      try {
        ...
        if (isMounted && data && data.length > 0) {
          ...
          setProductError('Không tìm thấy dữ liệu sản phẩm trên Supabase. Đang dùng dữ liệu demo.');
        }
      } catch (error) {
        console.error('Search page failed to load products', error);
        if (isMounted) {
          setProductError('Không thể tải dữ liệu sản phẩm từ Supabase. Đang dùng dữ liệu demo.');
        }
      } finally {
        if (isMounted) {
  ...
  const searchResults = useMemo(
    ...
    [query, selectedCategory, priceRange, sortBy, sourceProducts]
  );
        ...
        <div className="container mx-auto px-4 py-8">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
            <div>
              ...
              {loadingProducts && (
                <p className="text-sm text-gray-500 mt-2">Đang đồng bộ sản phẩm từ Supabase...</p>
              )}
            </div>
